...
```{r read_data}
# map shape file
nycd <- "resources/shiny_map_tutorial/data/nycd/nycd.shp"
...
homeless_tidy <- read.csv("https://raw.githubusercontent.com/HaozhongZheng/Shiny-Map-Tutorial/main/data/homeless_tidy.csv")
...
# save files
ggsave(filename = paste0(stringr::str_pad(i, 3, pad = "0"), ".png"), path = "resources/shiny_map_tutorial/figs/homeless/")
}
```
...
```{r gif}
imgs <- list.files("resources/shiny_map_tutorial/figs/homeless", full.names = TRUE)
lapply(imgs, image_read) %>%
  image_join() %>%
  image_animate(fps = 5) %>%  # can change speed of gif
  image_write("resources/shiny_map_tutorial/figs/homeless.gif")
```

![Alt Text](resources/shiny_map_tutorial/figs/homeless.gif)
...
```{r,echo=FALSE}
events = read.csv("https://raw.githubusercontent.com/HaozhongZheng/Shiny-Map-Tutorial/main/data/Events.csv")
