# cheatsheet_community_distribution
...
2. Boxplots
   ...
   2.4 Variable width boxplots:
       geom_boxplot(varwidth = TRUE)
   2.5 Multiple density histograms, ordered by median
...
3. Chapter 6 Rounding noraml distrubution
...
3.2 Density Curve + Noraml Curve
    3.2.1 How to draw
          library(tidyverse)
...
4. Chapter 7 Grammar of Graphics
    4.1 Layers
...
5. Chapter 8 Categorical Variables
    5.1 Types of data
...
6. Chapter 10 Tidying data
   Tidy: 1 variable per column, 1 observation per row
       
    
7. Chapter 11 Dependency relationships
    
    7.1 Interactive plot
        1) ggplot
        library(plotly)
        ggplotly(g)
...
8. Chapter 12 Continuous + Categorical 
   8.1 Graphiical data analysis with R
...
9. Parallel Coordinates (Multivariate continuous data)
   9.1 3D scatterplot
   3D scatterplot:
   ...
   9.2 Slope graph: ggplot2::geom_line()
...
10. Biplots
    ...
    10.3 Scatterplot of scaled data
       
    
11. Multivariate Categorical data

    11.1 Stacked bar chart
    ggplot(cases, aes(x = Age, fill = Favorite)) + 
      geom_bar() + 
      scale_fill_manual(values = c("#ff99ff", "#cc9966"))
    
    11.2 Group bar chart
    ggplot(cases, aes(x = Age, fill = Favorite)) +
      geom_bar(position = "dodge") +

      
    11.3 Cleveland dot plot
    1) Setting theme
       theme_dotplot <-
         theme_bw(16) +
...
14. Time Series
   14.1 Multiple time series: line
   ...
   14.4 label by day of week
    ...
    14.4 Highlight the abnormality
...
15. Dates
    ...
    15.2 Lubridate: ymd(), ydm(), mdy(), myd(), dmy(), dym()
         1) mdy
